...
import {
  newLiability, newAsset, newIncome, newExpense, newEvent, newScenario,
} from './model.js';
...
const readIncome = (row) => newIncome({
  id: row.dataset.id || undefined,
...
const readExpense = (row) => newExpense({
  id: row.dataset.id || undefined,
...
export function appendLiabilityRow(form, L = newLiability()) {
  return appendRow(form, '.liabilities-list', '.liability-template',
    row => writeLiability(row, L));
}
/**
 * @param {HTMLFormElement} form Scenario form.
...
export function appendAssetRow(form, A = newAsset()) {
  return appendRow(form, '.assets-list', '.asset-template',
    row => writeAsset(row, A));
}
/**
 * @param {HTMLFormElement} form Scenario form.
...
export function appendIncomeRow(form, inc = newIncome()) {
  return appendRow(form, '.incomes-list', '.income-template',
    row => writeIncome(row, inc));
}
/**
 * @param {HTMLFormElement} form Scenario form.
...
export function appendExpenseRow(form, ex = newExpense()) {
  return appendRow(form, '.expenses-list', '.expense-template',
    row => writeExpense(row, ex));
}
/**
 * @param {HTMLFormElement} form Scenario form.
...
export function appendEventRow(form, evt = newEvent()) {
  return appendRow(form, '.events-list', '.event-template',
    row => writeEvent(row, evt));
}
...
const readAllScenarios = () =>
  [...document.querySelectorAll('#scenarios form')].map(readScenario);
...
export function initInputs(onInput) {
  ...
  fire();
  scenariosEl.addEventListener('input', (e) => { if (e.target.closest('form')) fire(); });
