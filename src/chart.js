...
function _render(results, simDuration) {
  ...
  axesG.appendChild(
    el("line", { x1: PAD.left, y1: PAD.top, x2: PAD.left, y2: PAD.top + PH, ...stroke03 }),
  );
  ...
  results.forEach(({ series }, i) => {
    ...
    savingsG.appendChild(
      el("path", {
        d: buildSmoothPath(pairs, toX, ty, sumAssets),
        ...base,
  ...
  active.forEach((r, idx) => {
    ...
    if (idx > 0) x += sep;
    g.appendChild(
      el("line", { x1: x, y1: 0, x2: x + lineW, y2: 0, stroke: color, "stroke-width": "2" }),
    );
    x += lineW + gap;
